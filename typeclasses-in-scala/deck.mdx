...
<Notes>
  Let's try lists now.
  We know we can't change Scala's list type, so let's declare our own list wrapper, that wraps around a standard list, and make that encodable.
...
We can't convert the elements of the list to Json because we don't know what type they are!
We said our list can contain elements of any type `A`.
</Notes>
